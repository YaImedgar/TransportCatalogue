...
{
	void ReadRequests(tc::TransportCatalogue& transp_cat, std::istream& input)
	{
		...
		{
			RenderSettings(transp_cat, requests.at("render_settings").AsMap(), map_settings);
		}
	...
	std::pair<double, double> GetDoublePair(const json::Array& pair)
	{
		return std::make_pair(pair.front().AsDouble(), pair.back().AsDouble());
	}
	...
	void RenderSettings(tc::TransportCatalogue& transp_cat,
						...
						map_render::MapSettings& map_settings)
	{
		...
		if (nodes.count("bus_label_offset"))
			map_settings.bus_label_offset = GetDoublePair(nodes.at("bus_label_offset").AsArray());
		...
		if (nodes.count("stop_label_offset"))
			map_settings.stop_label_offset = GetDoublePair(nodes.at("stop_label_offset").AsArray());
		...
		if (nodes.count("color_palette"))
		{
			...
			for (const json::Node& color : nodes.at("underlayer_color").AsArray())
			{
				 ...
				 map_render::MapSettings& map_settings)
	{
		...
		for (const auto& query : nodes)
		{
			...
			if (query_str.compare("Map") == 0)
			{
				...
				map_dict.emplace("map", map_render::MapRender(map_settings));
